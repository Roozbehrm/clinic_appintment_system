...
class TestDoctor:
    ...
    def test_average_rating_reflects_reviews(
        self, doctor_user, patient_user, free_time_slot
    ):
    ...
    def test_mcc_must_be_unique(self, doctor_user, specialty):
        ...
        other_user = User.objects.create(
            phone_number="09129999999", email="doc2@example.com"
        )
...
class TestTimeSlot:
    ...
    def test_unique_together_prevents_same_doctor_date_start_time(
        self, doctor_user, free_time_slot
    ):
        with pytest.raises(IntegrityError):
            with transaction.atomic():
